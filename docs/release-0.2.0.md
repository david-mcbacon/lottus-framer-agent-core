...
| Signal | 0.1.0 baseline | 0.2.0 candidate | Release rule |
| --- | --- | --- | --- |
...
Supported versions are recorded in the README compatibility table. Desktop must pin immutable `0.2.0`, resolve it from the npm registry, and pass its packaged Electron acceptance. Roll back by pinning `0.1.0`, regenerating the host lockfile from npm, and repeating every host gate; never overwrite or unpublish an immutable Core artifact.
